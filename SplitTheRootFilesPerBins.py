...
def find_year(data_string):
    ...
    pattern = r'(2016APV|2016|2017|2018)'
...
def split_by_bins(input_file_name, output_dir):
    ...
    print ("This is year = ", find_year(outputfilename),"...Primary output filename.....",outputfilename)
    ...
    for dir_name in directories:
        if not input_file.GetDirectory(dir_name):
            print("Warning: Directory '{}' not found in the input file.".format(dir_name))
    ...
    for bin in range(1, n_bins + 1):
        ...
        print("Processing bin.....",bin,"...named..","{}/{}_mass_{}_{}.root".format(args.output_dir,outputfilename,str(bin_edges[bin-1]),str(bin_edges[bin])))
        #output_file_name = os.path.join(output_dir, "{}_mass_{}_{}.root".format(outputfilename,str(bin_edges[bin-1]),str(bin_edges[bin])))
        output_file = ROOT.TFile("{}/{}_mass_{}_{}.root".format(args.output_dir,outputfilename,str(bin_edges[bin-1]),str(bin_edges[bin])), "RECREATE")
        ...
        for dir_name in directories:
            ...
            if (args.SR): 
                if dir_name.split('_')[-2]=="tt":
                    output_dir = output_file.mkdir('tautau'+"_{}_{}_{}_SR".format(find_year(outputfilename),str(bin_edges[bin-1]),str(bin_edges[bin])))
                elif dir_name.split('_')[-2]=="lt":
                    output_dir = output_file.mkdir('leptau'+"_{}_{}_{}_SR".format(find_year(outputfilename),str(bin_edges[bin-1]),str(bin_edges[bin])))
                ...
                if dir_name.split('_')[-2]=="tt":
                    output_dir = output_file.mkdir('tautau'+"_{}_{}_{}_SB".format(find_year(outputfilename),str(bin_edges[bin-1]),str(bin_edges[bin])))
                elif dir_name.split('_')[-2]=="lt":
                    output_dir = output_file.mkdir('leptau'+"_{}_{}_{}_SB".format(find_year(outputfilename),str(bin_edges[bin-1]),str(bin_edges[bin])))
            ...
            for key in directory.GetListOfKeys():
                ...
                hist_copy = hist_copy.Rebin(1, hist_copy.GetName(), array('d', [bin_edges[bin-1],bin_edges[bin]]))
                ...
                if key.GetName().startswith("top") or key.GetName().startswith("others"):
                     if (bin_content==0):
                        print ("Bin content is zero for process...",key.GetName(),"...for bin..",bin)
                        ...
                        print ("Bin content is Negative = ",bin_content," for process...",key.GetName(),"...for bin..",bin)
                        if (key.GetName().startswith("top") and args.offAutoMCTop):
                            print ("Turn off automcstats for top")
                            hist_copy.SetBinContent(1, 1e-6) 
                            hist_copy.SetBinError(1, 0)
                     ...
                     #hist_copy.SetBinError(bin, bin_error)
                     if (bin_content<0):
                         print ("Bin content is Negative FOR SIGNALLL!!!! = ",bin_content," for process...",key.GetName(),"...for bin..",bin)
                         hist_copy.Sumw2()
                         #hist_copy.SetBinContent(bin, 1e-6)
                         hist_copy.SetBinContent(1, 1e-6)
                     elif (bin_content==0):
                        print ("Bin content is Zero FOR SIGNALLL!!!! = ",bin_content," for process...",key.GetName(),"...for bin..",bin)
                        hist_copy.Sumw2()
                        hist_copy.SetBinContent(1, 1e-6)
                     else:
                         hist_copy.SetBinContent(1, bin_content)
        ...
        output_file.Close()
        print("Created file:", "{}/{}_mass_{}_{}.root".format(args.output_dir,outputfilename,str(bin_edges[bin-1]),str(bin_edges[bin])))

    # Close the input file
    input_file.Close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Split histograms by bins into separate ROOT files.")
    parser.add_argument("--input_file",'-i',help="Path to the input ROOT file",required=True)
    parser.add_argument("--output_dir",'-o',help="Directory to save the output ROOT files",required=True)
    ...
    parser.add_argument("--offAutoMCTop",'-off',help="Turn off auto mc stats for top background",action='store_true')
