...
#variableSettingDictionary = {"X_m":"71, 750, 5010"}
...
class MakeHistograms(object):
	...
	def StandardDraw(self,theFile,
		...
		#bin_edges = [750, 900, 1050, 1200, 1350, 1500, 1650, 1800, 1950, 2200, 2450, 2700, 3200, 3700, 5500]
		#bin_edges = [750, 900, 1050, 1200, 1350, 1500, 1650, 1800, 1950, 2200, 2450, 2700, 3200, 5500]
		...
		#						additionalSelections,theWeight))
		#print("\n\n\n>>>>>>>>>>>>>>>>>>>>>>>")
		#print(variable + ">>" + histogramName, self.CreateCutString(standardCutString, additionalSelections, theWeight))
...
def main():
	parser = argparse.ArgumentParser(description='Create a input root files for combine DataCard.')    
	...
	#					default="(HTTvis_deltaR>0)&&(HTTvis_deltaR<1.5)&&(ngood_MediumJets==0)&&(Hbb_Loose==1)&&(X_m>=750)&&(X_m<=5010)&&(abs(Hbb_met_phi)>1)&&(HTTvis_m>20)")
	#
	parser.add_argument('--Channel',choices=["tt","et","mt","all","lt"], required=True)

	parser.add_argument('--update',help='Update exsiting ROOT file', action='store_true')

	parser.add_argument('--recreate',help='Recreate exsiting ROOT file', action='store_true')
...
#	if(args.Channel=="tt"):
#		additionalSelections = ["channel==0"]
#	elif(args.Channel=="et"):
#		additionalSelections = ["channel==1"]	 
#	elif(args.Channel=="mt"):
#		additionalSelections = ["channel==2"]	 	   
#	elif(args.Channel=="lt"):
#		additionalSelections = ["(channel==1) || (channel==2)"]	
	...
	for band in bandregion:
		...
		#For now exclude the Graviton and the additional signal files (This overrides L149)
		...
		fnames = list(set(glob.glob(args.Path + "/*.root"))-set(glob.glob(args.Path + "/*Graviton*.root"))-set(glob.glob(args.Path + "/QCD*.root")))
		...
		DatasetNameList=[]
		#List to hold the singal files names
		SignalNameList =[]
		...
		for file in fnames:
			...
			if (search("TTT", filename) or search("ST_", filename)):
		...
		print ("\n\n\n>>>SANITY CHECK>>>>")
		if((len(TopNameList)+len(OthersNameList)+len(Data_HistoList)) == len(DatasetNameList)):
			print("\n\n\nGrouping success...Sum of groups = ",(len(TopNameList)+len(OthersNameList)+len(Data_HistoList))," Total = ",len(DatasetNameList))
		else:
		...
		variable = "X_m"
		if year_unc == "2016": 
			Weight_signal = "xsWeight*boosteddeeptauWeight*L1PreFiringWeight_Nom*pileupcorrWeight*topptWeight*qcdWWeight*ewkWDeborahsWeight*qcdZTo2LWeight*ewkZDeborahsWeight*WnnloWeight*ZnnloWeight*btagmediumWeight*eleidWeight*elerecoWeight*muonidWeight*muonisoWeight*hpstauidWeight*metsfWeight*hbblooseWeight"
			#Weight_bkg = "FinalWeighting*bjetWeight_medium*Top_pt_rwt*qcdWWeight*ewkWDeborahsWeight*qcdZTo2LWeight*ewkZDeborahsWeight*WnnloWeight*ZnnloWeight"
			Weight_bkg = "xsWeight*boosteddeeptauWeight*L1PreFiringWeight_Nom*pileupcorrWeight*topptWeight*qcdWWeight*ewkWDeborahsWeight*qcdZTo2LWeight*ewkZDeborahsWeight*WnnloWeight*ZnnloWeight*btagmediumWeight*eleidWeight*elerecoWeight*muonidWeight*muonisoWeight*hpstauidWeight*metsfWeight" 
		elif year_unc == "2017":
			Weight_signal = "xsWeight*boosteddeeptauWeight*L1PreFiringWeight_Nom*pileupcorrWeight*topptWeight*qcdWWeight*ewkWDeborahsWeight*qcdZTo2LWeight*ewkZDeborahsWeight*WnnloWeight*ZnnloWeight*btagmediumWeight*eleidWeight*elerecoWeight*muonidWeight*muonisoWeight*hpstauidWeight*metsfWeight*hbblooseWeight"
			#Weight_bkg = "FinalWeighting*bjetWeight_medium*Top_pt_rwt*qcdWWeight*ewkWDeborahsWeight*qcdZTo2LWeight*ewkZDeborahsWeight*WnnloWeight*ZnnloWeight"
			Weight_bkg = "xsWeight*boosteddeeptauWeight*L1PreFiringWeight_Nom*pileupcorrWeight*topptWeight*qcdWWeight*ewkWDeborahsWeight*qcdZTo2LWeight*ewkZDeborahsWeight*WnnloWeight*ZnnloWeight*btagmediumWeight*eleidWeight*elerecoWeight*muonidWeight*muonisoWeight*hpstauidWeight*metsfWeight" 
		...
		if (band=="SR"):
			pnetcut =  " && (pnetmassnom>=100) && (pnetmassnom<=150)"
		elif (band=="SB"):
			pnetcut =  " && (((pnetmassnom>=0)  && (pnetmassnom<100)) || (pnetmassnom>150))"


		if(args.Channel=="tt"):
			standardCutString = "(channel==0)  && (ngood_MediumJets==0) && (hbbloose==1) && (X_m>=750) && (X_m<=5500) && (softdropmassnom>=30)" + pnetcut
		elif(args.Channel=="et"):
			standardCutString = "(channel==1)  && (ngood_MediumJets==0) && (hbbloose==1) && (X_m>=750) && (X_m<=5500) && (softdropmassnom>=30)" + pnetcut	 
		elif(args.Channel=="mt"):
			standardCutString = "(channel==2)  && (ngood_MediumJets==0) && (hbbloose==1) && (X_m>=750) && (X_m<=5500) && (softdropmassnom>=30)" + pnetcut	 	   
		elif(args.Channel=="lt"):
			standardCutString = "((channel==1) || (channel==2))  && (ngood_MediumJets==0) && (hbbloose==1) && (X_m>=750) && (X_m<=5500) && (softdropmassnom>=30)" + pnetcut	
		...
		if args.update:
			...
			if (band=="SR"):
				outputRootFile= ROOT.TFile(args.Output+'/'+args.year+'_allHists_SignalRegion_1fb_varbin.root', 'UPDATE')
			elif (band=="SB"):
				outputRootFile= ROOT.TFile(args.Output+'/'+args.year+'_allHists_SideBand_1fb_varbin.root', 'UPDATE')
			...
			if (band=="SR"):
				outputRootFile= ROOT.TFile(args.Output+'/'+args.year+'_allHists_SignalRegion_1fb_varbin.root', 'RECREATE')
			elif (band=="SB"):
				outputRootFile= ROOT.TFile(args.Output+'/'+args.year+'_allHists_SideBand_1fb_varbin.root', 'RECREATE')
		...
		def CreateClubAndStoreHists(DatasetNameList,variableToPlot,SignalNameList,Weight_signal,Weight_bkg,standardCutString,systematics=None,var=None,verbose=False):
			...
			for index in range(len(DatasetNameList)):
				if (search("MET", DatasetNameList[index]) or search("Run", DatasetNameList[index])):
					...
					DatasetObjects[DatasetNameList[index]].StandardDraw(DatasetObjects[DatasetNameList[index]].RootFileName,
						...
						None,
						#additionalSelections,
						DatasetNameList[index],theWeight=Weight_bkg)
			SignalObjects={}
			...
			for index in range(len(SignalNameList)):
				...
				SignalObjects[SignalNameList[index]].StandardDraw(SignalObjects[SignalNameList[index]].RootFileName,
					...
					None,
					#additionalSelections,
					SignalNameList[index],theWeight="0.001*"+Weight_signal)
			########################Signal-Histogram#############################
			...
			if(verbose):
				print ("Number of events in TTBar + ST (incl over and under) = ",Top_Histo.Integral(0,Top_Histo.GetNbinsX()+1))
				print ("Number of events in rest of the backgrounds (incl over and under) = ",Others_Histo.Integral(0,Others_Histo.GetNbinsX()+1))
				print ("Total Background (incl over and under) = ",Top_Histo.Integral(0,Top_Histo.GetNbinsX()+1) + Others_Histo.Integral(0,Others_Histo.GetNbinsX()+1))
				...
				print("\n\n\n")
				print ("Number of events for the bbtt 1.0 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-1000"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-1000"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 1.2 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-1200"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-1200"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 1.4 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-1400"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-1400"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 1.6 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-1600"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-1600"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 1.8 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-1800"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-1800"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 2.0 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-2000"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-2000"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 2.5 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-2500"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-2500"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 3.0 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-3000"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-3000"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 3.5 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-3500"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-3500"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 4.0 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-4000"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-4000"].HistogramName.GetNbinsX()+1))
				print ("Number of events for the bbtt 4.5 TeV (incl over and under) = ",SignalObjects["RadionTohhTohtatahbb_narrow_M-4500"].HistogramName.Integral(0,SignalObjects["RadionTohhTohtatahbb_narrow_M-4500"].HistogramName.GetNbinsX()+1))
				print("\n\n\n")
				print ("Number of events for the 1.0 TeV (incl over and under) = ",radion_1000.Integral(0,radion_1000.GetNbinsX()+1))
				print ("Number of events for the 1.2 TeV (incl over and under) = ",radion_1200.Integral(0,radion_1200.GetNbinsX()+1))
				print ("Number of events for the 1.4 TeV (incl over and under) = ",radion_1400.Integral(0,radion_1400.GetNbinsX()+1))
				print ("Number of events for the 1.6 TeV (incl over and under) = ",radion_1600.Integral(0,radion_1600.GetNbinsX()+1))
				print ("Number of events for the 1.8 TeV (incl over and under) = ",radion_1800.Integral(0,radion_1800.GetNbinsX()+1))
				print ("Number of events for the 2.0 TeV (incl over and under) = ",radion_2000.Integral(0,radion_2000.GetNbinsX()+1))
				print ("Number of events for the 2.5 TeV (incl over and under) = ",radion_2500.Integral(0,radion_2500.GetNbinsX()+1))
				print ("Number of events for the 3.0 TeV (incl over and under) = ",radion_3000.Integral(0,radion_3000.GetNbinsX()+1))
				print ("Number of events for the 3.5 TeV (incl over and under) = ",radion_3500.Integral(0,radion_3500.GetNbinsX()+1))
				print ("Number of events for the 4.0 TeV (incl over and under) = ",radion_4000.Integral(0,radion_4000.GetNbinsX()+1))
				print ("Number of events for the 4.5 TeV (incl over and under) = ",radion_4500.Integral(0,radion_4500.GetNbinsX()+1))				
				print("\n\n\n")
				#print ("Number of events for the 2 TeV (incl over and under) = ",Signal_Histo[1].Integral(0,Signal_Histo[1].GetNbinsX()+1))
				#print ("Number of events for the 3 TeV (incl over and under) = ",Signal_Histo[2].Integral(0,Signal_Histo[2].GetNbinsX()+1))
				#print ("Number of events for the 4 TeV (incl over and under) = ",Signal_Histo[3].Integral(0,Signal_Histo[3].GetNbinsX()+1))
		...
		CreateClubAndStoreHists(DatasetNameList,"X_m",SignalNameList,Weight_signal,Weight_bkg,standardCutString,verbose=True)
		...
		for systematic in event_weight_split_systematicslist:
			for var in variation_event_weight_split:
				WeightVar_signal = Weight_signal.replace(systematic,systematic+var)
				...
				print ("\n\nWeight string for ",systematic+var,"\n\n...bkg = ",WeightVar_bkg,"\n\n...sig = ", WeightVar_signal)
				print ("\n\nCutstring for ",systematic+var,"..= ",standardCutString)
				CreateClubAndStoreHists(DatasetNameList,"X_m",SignalNameList,WeightVar_signal,WeightVar_bkg,standardCutString,systematics=systematic,var=var)


		print("\n\n\n\n\n>>>Next store histograms for the split systematics for W and Z EWK weights and top pt reweighting >>>>>")
		
		event_weight_split_systematicslist = ["ewkWDeborahsWeight","ewkZDeborahsWeight","topptWeight"]
		...
		event_weight_split_systematicslist = ["isr","fsr","pdf","qcdscalerenorm","qcdscalefacto"]
		variation_event_weight_split = ["Up","Down"]
		...
		for systematic in event_weight_split_systematicslist:
			for var in variation_event_weight_split:
				WeightVar_signal = Weight_signal
				WeightVar_bkg = Weight_bkg
				standardCutString_Var = standardCutString.replace("pnetmassnom","pnetmass"+systematic+var)
				#print ("Weight string for ",systematic," ",var," is = ",WeightVar_signal, WeightVar_bkg)
				print ("\n\nWeight string for ",systematic+var,"\n\n...bkg = ",WeightVar_bkg,"\n\n...sig = ", WeightVar_signal)
				print ("\n\nCutstring for ",systematic+var,"..= ",standardCutString_Var)
				CreateClubAndStoreHists(DatasetNameList,"X_m",SignalNameList,WeightVar_signal,WeightVar_bkg,standardCutString_Var,systematics=systematic,var=var)
		...
		variation_event_weight_split = ["bcUp","bcDown","bc%sUp"%(args.year),"bc%sDown"%(args.year),"lightUp","lightDown","light%sUp"%(args.year),"light%sDown"%(args.year)]
		...
		systematicslist = ["boosteddeeptauWeight","L1PreFiringWeight_Nom","pileupcorrWeight","metsfWeight","eleidWeight","elerecoWeight","muonidWeight","muonisoWeight","hpstauidWeight","hbblooseWeight"]
			...
			for var in variation:
				...
				if var == "Up":
					if systematic=="hbblooseWeight":
						...
						WeightVar_signal = Weight_signal.replace(systematic,systematic.split("_")[-2]+"_Up")
						WeightVar_bkg =  Weight_bkg.replace(systematic,systematic.split("_")[-2]+"_Up")
					else:
						WeightVar_signal = Weight_signal.replace(systematic,systematic+var)
						WeightVar_bkg =  Weight_bkg.replace(systematic,systematic+var)		
				elif var == "Down":
					if systematic=="hbblooseWeight":
						...
						WeightVar_signal = Weight_signal.replace(systematic,systematic.split("_")[-2]+"_Dn")
						WeightVar_bkg =  Weight_bkg.replace(systematic,systematic.split("_")[-2]+"_Dn")
					else:
						WeightVar_signal = Weight_signal.replace(systematic,systematic+var)
						WeightVar_bkg =  Weight_bkg.replace(systematic,systematic+var)	
				...
				if (systematic=="L1PreFiringWeight_Nom"):
					print ("Storing the nuisance name in datacards : ",systematic.split('_')[-2])
					CreateClubAndStoreHists(DatasetNameList,"X_m",SignalNameList,WeightVar_signal,WeightVar_bkg,standardCutString,systematics=systematic.split('_')[-2],var=var)
				else:
					CreateClubAndStoreHists(DatasetNameList,"X_m",SignalNameList,WeightVar_signal,WeightVar_bkg,standardCutString,systematics=systematic,var=var)
		...
		systematicsUnortholist =  ["tesUp","tesDown","jesTotalUp","jesTotalDown","jerUp","jerDown",
							   "jesAbsoluteUp","jesAbsoluteDown",
							   "jesAbsolute_%sUp"%(year_unc),"jesAbsolute_%sDown"%(year_unc),
							   "jesBBEC1Up", "jesBBEC1Down",
							   "jesBBEC1_%sUp"%(year_unc),"jesBBEC1_%sDown"%(year_unc),
							   "jesEC2Up","jesEC2Down",
							   "jesEC2_%sUp"%(year_unc),"jesEC2_%sDown"%(year_unc),
							   "jesFlavorQCDUp","jesFlavorQCDDown",
							   "jesHFUp","jesHFDown",
							   "jesHF_%sUp"%(year_unc), "jesHF_%sDown"%(year_unc),
							   "jesRelativeBalUp","jesRelativeBalDown",
							   "jesRelativeSample_%sUp"%(year_unc),"jesRelativeSample_%sDown"%(year_unc),
							   "UnclustUp","UnclustDown"]

		# Function to modify the cut string for a given systematic
		def modify_cut_string(cut_string, systematic):
		    pattern = r"(\w+)([><=!]+)"
		    modified_string = re.sub(pattern, r"\1" + systematic + r"\2", cut_string)
		...
		def modify_weight_string(weight_string, systematic, exclusions):
		    pattern = r"\b(" + "|".join(re.escape(var) for var in exclusions) + r")\b"
		    modified_weights = [
		        f"{var}{systematic}" if not re.fullmatch(pattern, var) else var
		        for var in weight_string.split("*")
		    ]
		    return "*".join(modified_weights)
		excluded_weights = ["L1PreFiringWeight_Nom","xsWeight", "pileupcorrWeight", "topptWeight", "qcdWWeight", "ewkWDeborahsWeight", "qcdZTo2LWeight", "ewkZDeborahsWeight", "WnnloWeight", "ZnnloWeight"]

		for systematic in systematicsUnortholist:
			modified_cut_string = modify_cut_string(standardCutString, systematic)
			modified_weightbkg_string = modify_weight_string(Weight_bkg, systematic, excluded_weights)
			modified_weightsig_string = modify_weight_string(Weight_signal, systematic, excluded_weights)
			print ("\n\nWeight string for ",systematic,"\n\n...bkg = ",modified_weightbkg_string,"\n\n...sig = ", modified_weightsig_string)
			print ("\n\nCutstring for ",systematic,"..= ",modified_cut_string)
			print ("\n\nVariable to plot..= ","X_m%s"%(systematic))
			CreateClubAndStoreHists(DatasetNameList,"X_m%s"%(systematic),SignalNameList,modified_weightsig_string,modified_weightbkg_string,modified_cut_string,systematics=systematic,var="")
